...
class LinearClassifier(nn.Module):
    ...
    def forward(self, x):
        # flatten 
        # dimensionality of `x`: [batch_size, num_of_patches, patch_dim e.g. 768]
        # where `num_of_patches` is the N patches that make up the image
        # each of these patches is represented by a patch_dim vector, e.g. a vector of size 768 
        # but we need to have only the 
        # x = torch.mean(x, dim=1, dtype=x.dtype)
        if self.use_normalization: # you should probably not use normalization
                                   # because it gives better results due to dropout
            # add dropout
...
# Combine the Encoder that outputs feature vectors (a.k.a the Trunk)
# and the classifier Head (which outputs class id)
...
class LinearProbe():
    def __init__(self, args, logger):
        ...
        # You'll probably need to change the dataset here
