...
class LinearProbe():
    def __init__(self, args, logger):
        ...
        # ])
        # STL-10 specific transforms
        self.transform = transforms.Compose([
            transforms.Resize((self.crop_size, self.crop_size)),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.4467, 0.4398, 0.4066],
                                std=[0.2603, 0.2566, 0.2713])  # STL-10 specific normalization
        ])
        # Load CIFAR-10 datasets
        self.train_dataset_images = torchvision.datasets.STL10(root='./datasets', split='train', 
                                          download=True, transform=self.transform)
        self.val_dataset_images = torchvision.datasets.STL10(root='./datasets', split='test', 
                                        download=True, transform=self.transform)
    ...
    for itr in range(1,4):
        ...
        # ----------------------- AUTOMATIC LINEAR PROBING -----------------------
            ...
            for tarfile in sorted(tarfiles):
                ...
                eval_output = os.path.join(log_dir, 'ocls-jepa-STL10-' + basename + '.out') # + f'-ep{epoch}.out') 
                ...
                basename = os.path.basename(os.path.normpath(log_dir))
                temp_params['logging']['log_file'] = 'stats-STL10-' + basename + '-seed' + str(itr) + '.csv'
